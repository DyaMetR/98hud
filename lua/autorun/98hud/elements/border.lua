...
W98HUD:register(function()
  ...
  W98HUD.COMPONENTS:emptyWindow(title, 0, 0, w, h, W98HUD.FONTS.TITLE, config.bgCol1, colb, borderSize, colt, col1, col2, config.titleSize, config.bgEdge, config.bgLight, config.bgShadow, config.bgDarkShadow)
  W98HUD.COMPONENTS:windowControls(w - (4 + borderSize), 4 + borderSize, config.bgCol1, config.bgLight, config.bgShadow, config.bgDarkShadow, config.bgCol2, true, false, config.titleSize, W98HUD.FONTS.CAPTION)
end)
